...
Generates a service definition that runs either ``hle tunnel create ...`` (a single
...
def _systemd_list(*, user_mode: bool | None) -> None:
    ...
    over every tunnel, and `hle daemon install` now points people here to
...
def installed_services() -> list[tuple[str, bool]]:
    ...
    # `hle daemon install` now refuses to create, and the one an upgrade most
...
@service.group("install", cls=ModeGroup)
def install_group() -> None:
    """Install and start a background service.

    \b
    One of three things, each with only the options that apply to it:
      tunnel   a single tunnel, always up
      agent    the dashboard-driven agent, serving every endpoint you declare
      forward  a firepuncher forward, so a remote port is always local

    The old flag form (--agent, --fp, --service/--label) still works.
    """


@install_group.command(ModeGroup.LEGACY, hidden=True, cls=LegacyModeCommand)
@click.option(
    "--agent",
    "agent_mode",
    is_flag=True,
...
def install(
    ...
    Kept working for the unit files and install scripts already written
    against it. Each mode is now its own command, carrying only the options
    that apply to it:

    \b
      --service/--label  →  hle daemon install tunnel <label> <url>
      --agent            →  hle daemon install agent
      --fp               →  hle daemon install forward <agent> <target>
...
def _scope_options(f: F) -> F:
    """The scope flags every install mode shares."""
    f = click.option("--name", default=None, help="Override the unit/plist name")(f)
    f = click.option(
        "--user", "user_mode", is_flag=True, default=False, help="Install a per-user service"
    )(f)
    f = click.option(
        "--system", "system_mode", is_flag=True, default=False, help="Install a system-wide service"
    )(f)
    f = click.option("--run-as", "run_as", default=None, help="System service user")(f)
    return click.option("--start/--no-start", default=True, help="Enable + start it now")(f)
...
def install_tunnel(ctx: click.Context, /, label: str, url: str, **kwargs: Any) -> None:
    """Install one always-on tunnel.

    \b
    Example:
      hle daemon install tunnel ha http://localhost:8123
      hle daemon install tunnel ha http://localhost:8123 --system --run-as ian
    """
    ctx.invoke(install, service_url=url, label=label, **kwargs)


@install_group.command("agent")
@click.option("--relay-host", default=None, help="Relay host (default hle.world)")
@click.option("--relay-port", default=None, type=int, help="Relay port (default 443)")
@_scope_options
@click.pass_context
def install_agent(ctx: click.Context, /, **kwargs: Any) -> None:
    """Install the dashboard-driven agent.

    One process serves every endpoint you declare in the dashboard, so adding
    a service later needs no change on this machine.

    \b
    Example:
      hle daemon install agent
    """
    ctx.invoke(install, agent_mode=True, **kwargs)


@install_group.command("forward")
@click.argument("agent_name")
@click.argument("target")
@click.option("--port", "fp_port", default=None, type=int, help="Local port to bind")
@click.option("--bind", "fp_bind", default=None, help="Local address to bind")
@click.option("--relay-host", default=None, help="Relay host (default hle.world)")
@click.option("--relay-port", default=None, type=int, help="Relay port (default 443)")
@_scope_options
@click.pass_context
def install_forward(ctx: click.Context, /, agent_name: str, target: str, **kwargs: Any) -> None:
    """Install a permanent forward from a remote port to this machine.

    \b
    Example:
      hle daemon install forward rpi 22 --port 9922
      hle daemon install forward rpi 192.168.1.50:8006 --port 8006
    """
...
def logs(
    ...
    The command that was missing: a service reports "active (running)" while
    doing nothing useful — an agent with no token exits and is restarted
    forever — and the only way to see why was to know journalctl's flags, or
    where launchd was told to put the file.
